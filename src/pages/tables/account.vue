<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="query.id" placeholder="请输入ID" class="filter-item"
        style="width: 200px;"></el-input>
      <el-input v-model="query.name" placeholder="请输入名称" class="filter-item"
        style="width: 200px;"></el-input>
      <el-input v-model="query.email" placeholder="请输入邮箱" class="filter-item"
        style="width: 200px;"></el-input>
      <el-button
        v-waves
        class="filter-item"
  ...
  module.exports = {
    ...
    methods: {
      ...
      createData() {
        ...
        this.$refs["dataForm"].validate(valid => {
          if (valid) {
            ...
            this.loading = true;
            _axios.post("/api/account/add", this.temp).then(() => {
      ...
      updateData() {
        this.$refs["dataForm"].validate(valid => {
          if (valid) {
            ...
            _axios.post("/api/account/update", tempData).then(() => {
              this.dialogFormVisible = false;
              this.$message({
                message: "更新成功",
                type: "success",
      ...
      getData() {
        ...
        this.loading = true;
        _axios.get("/api/account/queryAll", this.query).then(res => {
          this.data = res.data;
