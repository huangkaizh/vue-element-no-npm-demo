  ...
  module.exports = {
    ...
    methods: {
      ...
      handleDownload() {
        ...
        loadScript("https://unpkg.com/xlsx@0.15.4/dist/xlsx.full.min.js").then(
          () => {
            ...
            XLSX.utils.book_append_sheet(wb, ws, "表格B");
            XLSX.writeFile(wb, "表格B.xlsx");
      ...
      createData() {
        ...
        this.$refs["dataForm"].validate(valid => {
          if (valid) {
            ...
            this.loading = true;
            _axios.post("/api/tableB/add", this.temp).then(() => {
      ...
      updateData() {
        this.$refs["dataForm"].validate(valid => {
          if (valid) {
            ...
            _axios.post("/api/tableB/update", tempData).then(() => {
              this.dialogFormVisible = false;
              this.$message({
                message: "更新成功",
                type: "success",
      ...
      getData() {
        ...
        this.loading = true;
        _axios.get("/api/tableB/queryAll", this.query).then(res => {
          this.data = res.data;
